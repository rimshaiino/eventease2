<div class="w-800">
    ...
    <div class="flex">
        ...
        @for ($i = 0; $i < 7; $i++)
            <div class="w-32">
                <div class="py-1 px-2 border border-gray-100 text-center italic">{{ $currentWeek[$i]['day'] }}</div>
                @for ($j = 0; $j < 21; $j++)
                    @if ($events->isNotEmpty())
                        @if (!is_null($events->firstWhere('start_date',$currentWeek[$i]['checkDay']. " " . Constant::EVENT_TIME[$j])))
                            ...
                            <div class="py-1 px-2 h-8 border border-gray-100 text-xs bg-purple-100 italic">{{ $eventName }}</div>
                            </a>
                            @if ( $eventPeriod > 0 )
                                @for($k = 0; $k < $eventPeriod; $k++)
                                <div class="py-1 px-2 h-8  bg-purple-100 italic"></div>
                                @endfor
                                @php $j += $eventPeriod @endphp
                            @endif
                        @else    
                            <div class="py-1 px-2 h-8 border border-gray-100 italic"></div>
                        @endif
                    @else
                        <div class="py-1 px-2 h-8 border border-gray-100 italic"></div>
                    @endif
